...
> **Best practice:** Keep source schema files and generated contracts in separate directories. Mixing them causes the engine to attempt re-ingesting its own output.
...
- **Python 3.9+** — required for pip installation
- **Rust toolchain** — required only if building from source ([install rustup](https://rustup.rs/))
- **maturin** — required only if building from source: `pip install maturin`
...
### `--action ingest` — Generate Data Contracts

Reads a source file (JSON, JSON Schema, XML, XSD, CSV, YAML) and emits structured data contracts.

### `--action dataquality` — Generate GX Test Suites

Reads a data contract and generates Great Expectations-compatible JSON suite files.

### `--action dqrun` — Execute GX Test Suites

The `dqrun` action executes Great Expectations test suites against actual data files. It requires:
- `--src` — path to the actual data file(s) to validate (`.csv`, `.json`, `.xlsx`, `.parquet`)
- `--tests` — path to a folder containing `*_suite.json` files (generated by `--action dataquality`)
- `--output` — where to write validation results
- `--type greatexpectations`

Optional:
- `--contracts` — path to data contracts (`.contract.json` files) for auditing and pipeline traceability

**Prerequisites**: `great-expectations` and `pandas` must be installed:
...
**Example**:
```bash
# Generate suites first
aytch --action dataquality --src schema.json --output ./expectations --type greatexpectations

# Run suites against data
aytch --action dqrun \
  --src ./data/transactions.csv \
  --tests ./expectations/schema \
  --output ./results \
  --type greatexpectations

# With contracts for auditing
aytch --action dqrun \
  --src ./data/transactions.csv \
  --tests ./expectations/schema \
  --contracts ./contracts \
...
**Output**: Per-suite `*_results.json` files and a `validation_summary.json` in the output directory.

### CLI Examples

```powershell
# Ingest a JSON Schema -> all output formats
.\aytch.exe --action ingest --src examples\sample_json_schema.json --output .\contracts

# Ingest an XSD with owner/domain metadata -> YAML only
.\aytch.exe --action ingest --src examples\sample.xsd --output .\contracts --format yaml `
            --owner "hr-team" --domain "hr"

# Ingest a CSV data dictionary -> JSON contract
.\aytch.exe --action ingest --src examples\sample_data_dictionary.csv --output .\contracts --format json

# Generate GX suites for a single contract, skip baseline tests
.\aytch.exe --action dataquality --src .\contracts\schema.contract.json --output .\expectations --no-baseline

# Generate only specific baseline suites
.\aytch.exe --action dataquality --src .\contracts --output .\expectations `
            --suites "data_validity,data_completeness,criminal_background_check"

# Run GX suites against a CSV data file
.\aytch.exe --action dqrun --src .\data\transactions.csv --tests .\expectations\schema `
            --output .\results --type greatexpectations

# Full pipeline: ingest -> dataquality -> dqrun
.\aytch.exe --action ingest --src .\schemas\ --output .\contracts --recursive
.\aytch.exe --action dataquality --src .\contracts --output .\expectations --recursive
.\aytch.exe --action dqrun --src .\data\ --tests .\expectations --contracts .\contracts `
            --output .\results --type greatexpectations --recursive
```
...
The `--action dataquality` command drives the `data-quality-core` engine, which produces **Great Expectations-compatible JSON suite files** ready to load directly into a GX checkpoint.
...
data-quality-core
  ...
  └── GX Output Writers          (JSON suites + summary.csv + manifest.json)
     │
     ▼
Great Expectations Suite Files (*_suite.json)
     │
     ▼  --action dqrun
GX Executor (Python + great-expectations + pandas)
  ├── Loads *_suite.json files from --tests
  ├── Loads data file(s) from --src (.csv / .json / .xlsx / .parquet)
  ├── Links data contracts from --contracts (optional, for auditing)
  └── Writes per-suite *_results.json + validation_summary.json
     │
     ▼
Validation Results
```
